...
**Prepared:** 2026-07-28. **Revised: 2026-08-08**, America/Chicago
...
**Sections 38–46 cover 2026-08-01/02; sections 47–52 cover 2026-08-08**
and are the most important part for anyone picking the project up now.
**§52 lists what was agreed on 2026-08-08 but NOT yet built** - read it
before trusting the auction output.
**Test count:** 378 (311 on July 28, 365 on Aug 2).
...
> There are 378 passing offline tests, a synthetic end-to-end run, and an
...
## 47. 2026-08-08 — PSA qualifier exclusion (OC, MK, ST...)

A PSA 9 (OC) is off-centre and trades far below a clean 9, and one sat on
the Action sheet: "1986 Fleer Michael Jordan Rookie #57 PSA 9 (OC)".

Qualifiers cannot go in `exclude_keywords`, which lowercases and matches
substrings - "oc" is inside block, occasion, Knocks, Rockies, chocolate and
Ochoa. **`filters.exclude_qualifiers`** is a separate mechanism: whole
token, CAPITALS required, brackets optional (`main._qualifier_excluded`).
Config ships OC, MK, ST, MC, OF, PD.

Applied to listings, comps AND targeted comp pools - a qualified card in
the comp pool drags fair value down as wrongly as one in the listing pool
inflates an edge.

---

## 48. 2026-08-08 — full scans are auctions-only

Decision: `Run Scan.command` reports AUCTIONS. BINs belong to
`Run BIN Sweep.command`, which already exists and runs on its own
schedule. `scanning.auctions_only: false` restores the old mixed workbook.

Two layers:
1. **Fetch**: mode "all" is translated to the existing "auctions" mode, so
   sources are never asked for fixed-price listings at all (~half the
   network calls - fetch was 593s of the 27-minute run).
2. **Relevance loop**: any fixed-price listing that arrives anyway (HTML
   fallbacks that cannot express the filter) is dropped at the top, before
   identity work, comp statistics or guide lookups. They were ~68% of
   everything valued (8,569 of 12,651 rows on 2026-08-08).

---

## 49. 2026-08-08 — the scan is ~3.5x faster, and why

27m04s run, profiled: fetch 593s, valuation+db 912s, closer 96s.

Three changes (commit d55e88d):
1. **Auctions-only** (§48) - removes ~2/3 of valuation work and ~1/2 of
   fetch.
2. **Guide CSV search: shortlist on rare tokens, rank on all.** 63% of
   valuation, 48ms/call, because "comic" walked 347,716 postings. NOTE:
   simply IGNORING common tokens was tried first and was WRONG - card
   numbers are common tokens ("2" is in 45,064 products) and decisive, so
   "electrode #2" started answering Electrode #101. The shortlist/rank
   split keeps every token in the ranking. Verified at quote level:
   299/300 identical, zero different values.
3. **Identity cache**: score_candidate/_quote_from_rows called
   CardIdentity.from_text directly, bypassing the memoised identity_of -
   11,958 parses per 150 evaluations. Now routed through the cache.

30.2ms -> 9.0ms per listing. Estimated full run: **~7-8 minutes**.

Caveats recorded honestly: fetch is network-bound (halved calls, not
halved seconds), and the 27-minute baseline was itself flattered by an
open pricecharting breaker skipping 2,551 guide lookups. Parallelising
valuation was proposed and REJECTED: the GIL makes threads useless for
CPU-bound work, and profiling showed one slow function, not missing
parallelism.

---

## 50. 2026-08-08 — grade bands on structured targets

Andrew's rule: a grade one lower and two higher than asked is still the
card he wants. `grade: 8` means 7-10; `grade_band: [1, 5]` is explicit.
(`targets.grade_band`, enforced in `structured_target_mismatch` via
grade_min/grade_max on the entry.)

The band is also a RUNTIME feature: the old code expanded every grade into
its own eBay search, so 35 cards x 5 grades would have been 175 queries.
A band is ONE query whose results are filtered. The grade is deliberately
left OUT of the query text - asking eBay for "PSA 8" hides the PSA 7 and
9 copies the band also wants. When a band is in force it replaces
grade_conflict, which would otherwise read the grade-free query as
"wants ungraded".

---

## 51. 2026-08-08 — Andrew's exact card list (49 structured targets)

`sports_targets` (30), new `pokemon_targets` (16) and new `other_targets`
(3) all share the same machinery via
`sports_target_entries(config, key=...)`.

Band selection follows Andrew's $10k rule: PSA 6 under ~$10k -> hunt 6-10;
over -> hunt 1-5 (Jordan '84 Star, Ruth '33 Goudey, Cobb T206 Green, Anson
1889, Mikan '48, Ted Williams '39).

Details that will bite if forgotten:
- **Card numbers only where confident.** A wrong number makes
  structured_target_mismatch reject every real listing SILENTLY.
- **Jungle/Fossil print the same Pokemon twice**: Snorlax #11 holo / #27
  non-holo, Gengar #5/#20, Dragonite #4/#19. Numbers were read from
  guide_csv, not memory. The watchlist wants holos.
- **Hank Aaron has no 1952 card** - his rookie is 1954 Topps #128.
- **Superman 1940 is the Gum Inc CARD #1**, not the comic; Andrew wants #1
  only. Walt Disney is the 1931 Wills Cinema Stars Mickey card. Luke
  Skywalker is 1977 Topps. Tiger "Authentic Stars" is the 2001 SP
  Authentic signed rookie.
- **Tiger UDA flags are memorabilia watchlist queries** (US Open priority),
  NOT card targets - the §42 gate keeps card guides away from them.
- **Super Mario and Batman carry `value_floor_override: 1000`** - Andrew
  wants only $1k+ cards from those queries.
- The **1996 No Rarity set** is a SET need with grade_min 8 and a zero
  floor: any card completes the set.

---

## 52. Agreed but NOT yet implemented (2026-08-08 plan)

From the numbered plan Andrew approved:
1. **Auctions judged on dollar EDGE, not ROI.** The ROI ceiling rejected
   303 auction rows at a median 6,289% because an early auction's ratio
   against a $10 opening bid is meaningless. Andrew chose "judge on edge"
   over exempting or raising the ceiling. NOT BUILT YET.
2. **72h auction horizon review** - naturally follows (1). NOT DONE.
3. **$500 floors revisit** - they cut 11,418 rows in the last run, 40x
   more than the ROI ceiling. Deliberately postponed.

Andrew intends a test run before further changes. When reading its
workbook remember the ROI ceiling is still in force for auctions.

---

## 53. Fresh-instance startup checklist
...
## 54. “Picked up where we left off” test
...
- 378 tests and the synthetic demo pass (311 on July 28).
...
a hard prohibition on negative-EV BIN Telegram alerts; as of 2026-08-08 it is auctions-only, ~3.5x faster, carries 49 exact
grade-banded targets from Andrew's own list, and drops PSA-qualified (OC)
slabs; its binding constraints are that **sports guide coverage is one
set** (§38, §46), that **no live run has accepted any of this work**, and
that **the ROI ceiling still wrongly rejects early auctions** until §52's
edge-based gate is built.
